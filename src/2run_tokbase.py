...
def parse_args():
    ...
    parser.add_argument(
        "--span_val_file", type=str, default='/home/vv2116/capstone_data/altlex_test_subtask2.csv', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--ratio",
    ...
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default="tanfiona/unicausal-tok-baseline",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default = True,
        help="Get predictions on validation/tests set.",
    )
...
def main():
    ...
    if args.do_predict:
        ...
        # Save Predictions
        # op_file_vv = args.pair_val_file.split("/")[-1].split('.')[0]
        op_file_vv = args.dataset_id
        eval_pspan_index_col = span_datasets["span_validation"]["index"]
        ((span_datasets["span_validation"]).data.to_pandas()).to_csv(f"./results/subtask2_{op_file_vv}_tokens.csv",index=False)
        assert(len(all_preds)==len(all_refs))
        assert(len(eval_pspan_index_col)==len(all_preds))
        with open(os.path.join(args.output_dir,f"subtask2_{op_file_vv}_predictions.txt"), "w") as writer:
